// Card geometry for the flag intro ring.
...
export const RING_COPIES = 2;   // repeat the day's flags around the ring → 10 cards, tighter band
export const RADIUS_PAD = -6;   // gap: slightly under the touching baseline → edge-to-edge
export const RING_CARD_W = 256; // nominal card width — drives the ring radius
export const RING_CARD_H = 112;
...
/** Bend radius of a nominal card. Each card is pushed back by its own bend radius
 *  (see `pose`), so the curved surface lands on the ring radius itself. */
...
export const cardBox = (svg: string, scale = 1, mode: FitMode = CARD_FIT_MODE): CardBox => {
  ...
  // so each card is pulled back by exactly that much: the surface then sits on the
  // ring radius, not a bend-radius further out (which made the whole band balloon
  // and the near cards loom over the card).
  const pose = (w: number) => {
    const bendR = (w / 2) / Math.tan((CARD_BEND / 2) * Math.PI / 180);
    return { bend: CARD_BEND, bendR, zOffset: -bendR };
  };
